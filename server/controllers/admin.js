...
const getDashboardStats = TryCatch(async (req, res) => {
  ...
  const last7DaysChats = await Chat.find({
    createdAt: {
      $gte: last7Days,
      $lte: today,
    },
  ...
  const chats = new Array(7).fill(0);
  const users = new Array(7).fill(0);
  ...
  last7DaysChats.forEach((chat) => {
    const indexApprox =
      (today.getTime() - chat.createdAt.getTime()) / dayInMiliseconds;
    const index = Math.floor(indexApprox);
  ...
  last7DaysUsers.forEach((user) => {
    const indexApprox =
      (today.getTime() - user.createdAt.getTime()) / dayInMiliseconds;
    const index = Math.floor(indexApprox);
